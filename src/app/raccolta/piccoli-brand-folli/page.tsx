...
import {
  SMALL_BRANDS_FOLDER_DESCRIPTION,
  SMALL_BRANDS_FOLDER_SLUG,
  SMALL_BRANDS_FOLDER_TITLE,
} from "@/lib/small-brands-curated";
import { ensureDatabaseReady, getFolderCampaigns } from "@/lib/db";

export const dynamic = "force-dynamic";

export default async function SmallBrandsCuratedPage() {
  const { user, profile } = await requireCompleteProfile();
  await ensureDatabaseReady();
  const items = await getFolderCampaigns(SMALL_BRANDS_FOLDER_SLUG, user.id);
  ...
  return (
    <>
      ...
      <div className="mx-auto max-w-[960px] px-6 pb-24 pt-8">
        ...
        <h1 className="section-title mb-4">{SMALL_BRANDS_FOLDER_TITLE}</h1>
        ...
        <div className="glass-panel mb-10 p-6 md:p-8">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {[
              {
                title: "Persone",
                text: "Founder, agenzia piccola, team lean e produzione indie.",
              },
              {
                title: "Agenzia e creativi",
                text: "Idea folle, stunt, tono e meccanica virale.",
              },
              {
                title: "Spiegazione progetto",
                text: "Perché un brand piccolo può battere i giganti con l'insight giusto.",
              },
              {
                title: "Altre info",
                text: "Premi, share, vendite e riconoscimento del pubblico.",
              },
            ].map((block) => (
              <div
                key={block.title}
